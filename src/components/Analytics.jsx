...
const Analytics = () => {
  ...
  const getMetricsByPeriod = (period) => {
    const metricsData = {
      week: [
        ...
        { id: 3, label: 'Orders', value: '41', change: '+4%', trend: 'up', period: 'vs last week' },
        { id: 4, label: 'Avg. Order Value', value: '₱820', change: '-2%', trend: 'down', period: 'vs last week' },
        { id: 5, label: 'Customer Retention', value: '75%', change: '+1%', trend: 'up', period: 'vs last week' }
      ],
      month: [
        ...
        { id: 3, label: 'Orders', value: '198', change: '+11%', trend: 'up', period: 'vs last month' },
        { id: 4, label: 'Avg. Order Value', value: '₱850', change: '+5%', trend: 'up', period: 'vs last month' },
        { id: 5, label: 'Customer Retention', value: '78%', change: '+3%', trend: 'up', period: 'vs last month' }
      ],
      year: [
        ...
        { id: 3, label: 'Orders', value: '2,102', change: '+16%', trend: 'up', period: 'vs last year' },
        { id: 4, label: 'Avg. Order Value', value: '₱895', change: '+9%', trend: 'up', period: 'vs last year' },
        { id: 5, label: 'Customer Retention', value: '82%', change: '+7%', trend: 'up', period: 'vs last year' }
      ]
    };
    return metricsData[period];
  };

  const getOrderDistributionByPeriod = (period) => {
    const data = {
      week: { petShop: 26, petMenu: 15 },
      month: { petShop: 124, petMenu: 74 },
      year: { petShop: 1322, petMenu: 780 }
    };
    return data[period];
  };

  const getPopularOrdersByPeriod = (period) => {
    const data = {
      week: [
        { name: 'JerHigh Dog Treats 70g', count: 18, category: 'Pet Shop' },
        { name: 'Chicken Rice Bowl', count: 12, category: 'Pet Menu' },
        { name: 'CatCare Cat Food 1kg', count: 10, category: 'Pet Shop' },
        { name: 'Frozen Paw Bites', count: 8, category: 'Pet Menu' }
      ],
      month: [
        { name: 'JerHigh Dog Treats 70g', count: 71, category: 'Pet Shop' },
        { name: 'Chicken Rice Bowl', count: 55, category: 'Pet Menu' },
        { name: 'CatCare Cat Food 1kg', count: 49, category: 'Pet Shop' },
        { name: 'Frozen Paw Bites', count: 41, category: 'Pet Menu' }
      ],
      year: [
        { name: 'JerHigh Dog Treats 70g', count: 840, category: 'Pet Shop' },
        { name: 'Chicken Rice Bowl', count: 603, category: 'Pet Menu' },
        { name: 'CatCare Cat Food 1kg', count: 542, category: 'Pet Shop' },
        { name: 'Frozen Paw Bites', count: 496, category: 'Pet Menu' }
      ]
    };
    return data[period];
  };
  ...
  const orderDistribution = getOrderDistributionByPeriod(activePeriod);
  const popularOrders = getPopularOrdersByPeriod(activePeriod);
      ...
      <div className="analytics-orders-grid">
        <div className="insights-card orders-distribution-card">
          ...
          <div className="orders-donut-wrap">
            <div className="orders-donut" style={{ background: `conic-gradient(#f53799 0 ${(orderDistribution.petShop / (orderDistribution.petShop + orderDistribution.petMenu)) * 360}deg, #5ce1e6 ${(orderDistribution.petShop / (orderDistribution.petShop + orderDistribution.petMenu)) * 360}deg 360deg)` }}>
              <div className="orders-donut-center">{orderDistribution.petShop + orderDistribution.petMenu}</div>
            </div>
            <div className="orders-distribution-legend">
              <div><span className="legend-dot" style={{ background: '#f53799' }}></span> Pet Shop: {orderDistribution.petShop} ({((orderDistribution.petShop / (orderDistribution.petShop + orderDistribution.petMenu)) * 100).toFixed(1)}%)</div>
              <div><span className="legend-dot" style={{ background: '#5ce1e6' }}></span> Pet Menu: {orderDistribution.petMenu} ({((orderDistribution.petMenu / (orderDistribution.petShop + orderDistribution.petMenu)) * 100).toFixed(1)}%)</div>
            </div>
          </div>
        </div>
